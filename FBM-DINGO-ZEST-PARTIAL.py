...
def rename_column_final(merged_partial):
    ...
    merged_partial.rename(columns={"FBM CASH IN": "FBM"}, inplace=True)
    merged_partial.rename(columns={"FBM SPACE": ""}, inplace=True)
    merged_partial.rename(columns={"FBM POUCH": ""}, inplace=True)
    merged_partial.rename(columns={"DINGO VLT": ""}, inplace=True)
    merged_partial.rename(columns={"Dingo Subtracted": "DINGO"}, inplace=True)
    merged_partial.rename(columns={"DINGO SPACE": ""}, inplace=True)
...
# functipm calls of ZEST:

# SELECT IMPORTANT COLUMNS, CHANGE COLUMN NAMES, SELECT ONLY NUMBER TYPES IN ROWS, AND REMOVE COMMAS
zest = select_column(zest, 0 ,6)
zest = change_column_name(zest, "DINGO VLT", "Dingo Subtracted")
zest = zest[zest["DINGO VLT"].apply(lambda num: num.replace(",","").replace(".","").isnumeric())]
zest = zest[zest["Dingo Subtracted"].apply(lambda num: num.replace(",","").replace(".","").isnumeric())]
zest["Dingo Subtracted"] = zest["Dingo Subtracted"].str.replace(",","", regex=False)

# CONVERT COLUMNS TO INT AND FLOAT
convert_to_int(zest, zest.columns[0])
convert_to_float(zest, zest.columns[1])
convert_to_int(zest, zest.columns[1])

# TO REMOVE DUPLICATES
zest = zest.drop_duplicates(subset="DINGO VLT")

# ZEST VLT SHOULD ALWAYS BE 1 TO 38
zest_vlt = range(1, 39)
complete_vlt = pd.DataFrame({"DINGO VLT": zest_vlt})
zest = complete_vlt.merge(zest, on="DINGO VLT", how="left")

# SUBTRACT ZEST CASH IN FROM ZEST GRAND TOTAL
zest["Dingo Subtracted"] = zest["Dingo Subtracted"] - zest_grand_total[11]

# SELECT ROWS WITH 2300 AND ABOVE, RESET INDEX AND CONVERT TO INTEGER
zest = zest[zest["Dingo Subtracted"] > 2300].reset_index(drop=True)
convert_to_int(zest, zest.columns[1])
...
# ZEST TITLE
dingo_partial.loc[len(dingo_partial)] = ["", "ZEST",]
#zest_title = {"DINGO VLT" : "ZEST", "Dingo Subtracted": 0}

# MERGE DINGO AND ZEST

dingo_zest = pd.concat([dingo_partial, zest], axis=0).reset_index(drop=True)
#dingo_zest.to_csv("c:\\Users\\Dell\\Desktop\\BRYAN FILES\\BRYAN PROGRAM\\dingo_zest.csv", index=False)




# COMBINE DATA OF fbm_partial & dingo_partial
merged_partial = pd.concat([fbm_partial, dingo_zest], axis=1)
...
# DINGO AND ZEST POUCH NUMBERING
pouch_number = []


list_of_dingo = list(merged_partial["Dingo Subtracted"])

for item in list_of_dingo:
    if item == "ZEST":
        pouch_number.append("")
    else:
        pouch_number.append(number_start)
        number_start += 1

merged_partial["DINGO POUCH"] = pouch_number

# REMOVE ZEROS FROM DINGO
convert_dingo_blankspaces(merged_partial)


merged_partial["DINGO POUCH"] = np.where(merged_partial["Dingo Subtracted"] == "", "", merged_partial["DINGO POUCH"])
...
try:
    merged_partial.to_csv("c:\\Users\\Dell\\Desktop\\BRYAN FILES\\BRYAN PROGRAM\\FBM-DINGO-ZEST.csv", index=False)
...
#pyinstaller --onefile --windowed --icon=3_SUPPLIERS.ico FBM-DINGO-ZEST-PARTIAL.py
